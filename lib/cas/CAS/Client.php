    ...
    */
    ...
    public function handleLogoutRequests($check_client=true, $allowed_clients=false)
    {
        ...
        if ($check_client) {
			...
			$client =isset($allowed_clients[0])?$allowed_clients[0]:$client;
    ...
    */
    ...
    private function _getServerUrl()
    {
        ...
        if (!strpos($server_url, ':')) {
            ...
            if ( ($this->_isHttps() && $server_port!=443 &&!empty($server_port))
                || (!$this->_isHttps() && $server_port!=80 &&!empty($server_port))
            ) {
                $server_url .= ':';
    ...
    private function _renameSession($ticket)
    {
        ...
        if ($this->getChangeSessionID()) {
            if (!empty($this->_user)) {
                ...
                $_SESSION['milanoo_cas_ticket'] = $ticket;//保存st到session
            } else {
