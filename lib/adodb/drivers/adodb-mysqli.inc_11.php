...
class ADODB_mysqli extends ADOConnection {
	...
	{	  
		...
		$this->write_content();
		mysqli_autocommit($this->write_link, false);
		$this->Execute('BEGIN');
		return true;
	}
	...
	function CommitTrans($ok=true) 
	{
		...
		mysqli_autocommit($this->write_link, true);
		return true;
	}
	...
	function _insertid()
	{
	  ...
	  $result = mysqli_insert_id($this->write_link);
	  if ($result == -1){
	...
	{
	  $result =  @mysqli_affected_rows($this->write_link);
	  if ($result == -1) {
	...
	function GenID($seqname='adodbseq',$startID=1)
	{
		...
		if ($rs) {
			$this->genID = mysqli_insert_id($this->write_link);
			$rs->Close();
		} else
			$this->genID = 0;
...
//	    $this->_connectionID = $this->mysqli_resolve_link($this->_connectionID);
	...
	global $ADODB_COUNTRECS;
		...
		*/
		...
		if ($this->multiQuery) {
			$sql=trim($sql);
			$read_sql_pos=preg_match("/^(select|set)/i",$sql);
			if($read_sql_pos)
			{
				...
				$rs = mysqli_multi_query($this->write_link, $sql.';');
				if ($rs) {
					$rs = ($ADODB_COUNTRECS) ? @mysqli_store_result( $this->write_link ) : @mysqli_use_result( $this->write_link );
					return $rs ? $rs : true; // mysqli_more_results( $this->_connectionID )
				}		
			}
		} else {
			...
			if($read_sql_pos)
			{
				...
				$this->write_content();
				$rs = mysqli_query($this->write_link, $sql, $ADODB_COUNTRECS ? MYSQLI_STORE_RESULT : MYSQLI_USE_RESULT);
		...
		function write_content() {
			global $db_host_write, $db_user_write, $db_pass_write, $db_name_write;
						//		print_r($db_read_config);	
			if ($this->write_link) {
				return $this->write_link;
			}
			$write_link = new mysqli ($db_host_write,$db_user_write,$db_pass_write,$db_name_write);
			if (! $write_link) {
				die ( 'Connect Error (' . mysqli_connect_errno () . ') ' . mysqli_connect_error () );
			}

			$write_link->query ( "SET NAMES utf8" );
			$this->write_link = $write_link;
